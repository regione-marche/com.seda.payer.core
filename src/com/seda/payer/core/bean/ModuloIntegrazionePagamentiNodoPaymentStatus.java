...
public class ModuloIntegrazionePagamentiNodoPaymentStatus extends Lifecycle implements Serializable {
    ...
    public ModuloIntegrazionePagamentiNodoPaymentStatus() { 
    }

    public ModuloIntegrazionePagamentiNodoPaymentStatus(ResultSet data) throws SQLException {
    	if (data == null)
    		return;

    	Calendar cal = Calendar.getInstance();
    	
    	chiaveTransazione = data.getString("MPN_KTRAKTRA");
    	gruppoTentativiNotifica = data.getInt("MPN_NMPSGRUP");
        numeroTentativoNotifica = data.getInt("MPN_NMPSCORR");
        modalitaNotifica = data.getString("MPN_CMPSMODA");
    	esitoTransazione = data.getString("MPN_FMPSFESI");
    	idPortale = data.getString("MPN_KMPSPORT");
    	numeroOperazione = data.getString("MPN_KMPSOPER");
    	xmlPaymentStatus = data.getString("MPN_CMPSPSTS");
    	xmlPaymentData = data.getString("MPN_CMPSPDAT");
    	xmlCommitMessage = data.getString("MPN_CMPSCMSG");
        esitoNotifica = data.getString("MPN_CMPSESIT");
        S2SResponseHtmlStatusCode = data.getString("MPN_CMPSCODE");
        S2SResponseMessage = data.getString("MPN_DMPSRESP");
        parametriOpzionali1 = data.getString("MPN_DMPSOPT1");
        parametriOpzionali2 = data.getString("MPN_DMPSOPT2");
        cal.setTimeInMillis(data.getTimestamp("MPN_GMPSGAGG").getTime());
        dataUltimoAggiornamento = cal;
        operatoreUltimoAggiornamento = data.getString("MPN_CMPSCOPE");
        xmlSendRT = data.getString("MPN_CMPSCXSR") != null ? data.getString("MPN_CMPSCXSR") : null;
    }
    
    public ModuloIntegrazionePagamentiNodoPaymentStatus(ResultSet data, boolean bBatch, boolean bUpdate) throws SQLException 
    {
    	this(data);
    	
    	if (bBatch)
    	{
    		//parametri recuperati solo per la stored PYMPNSP_BATCH
        	xmlPaymentRequestMIP = data.getString("MIN_CMIPPREQ");
        	codicePortaleEsternoMIP = data.getString("MIN_CMIPPORT");
        	parametriOpzionali1MIP = data.getString("MIN_DMIPOPT1");
    	}
    	if (bUpdate)
    	{
    		//parametro recuperato solo per la stored PYMPNSP_UPD
...
//	YLM PG22XX07 INI
	...
	/**
	 * @param xmlSendRT the xmlSendRT to set
	 */
...
//		YLM PG22XX07 INI
		arg.setString(17, this.xmlSendRT != null ? xmlSendRT : "");
//		YLM PG22XX07 FINE		
		
		arg.registerOutParameter(18, Types.INTEGER); //gruppo tentativi
		arg.registerOutParameter(19, Types.INTEGER); //numero tentativo
	}
	...
	public void onUpdate(CallableStatement arg) throws SQLException {
		...
		int i = 4;
		if(this.modalitaNotifica != null) {
	    	arg.setString(i, this.modalitaNotifica);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.esitoTransazione != null) {
	    	arg.setString(i, this.esitoTransazione);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.idPortale != null) {
	    	arg.setString(i, this.idPortale);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.numeroOperazione != null) {
	    	arg.setString(i, this.numeroOperazione);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.xmlPaymentStatus != null) {
	    	arg.setString(i, this.xmlPaymentStatus);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.xmlPaymentData != null) {
	    	arg.setString(i, this.xmlPaymentData);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.xmlCommitMessage != null) {
	    	arg.setString(i, this.xmlCommitMessage);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.esitoNotifica != null) {
	    	arg.setString(i, this.esitoNotifica);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.S2SResponseHtmlStatusCode != null) {
	    	arg.setString(i, this.S2SResponseHtmlStatusCode);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.S2SResponseMessage != null) {
	    	arg.setString(i, this.S2SResponseMessage);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.parametriOpzionali1 != null) {
	    	arg.setString(i, this.parametriOpzionali1);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.parametriOpzionali2 != null) {
	    	arg.setString(i, this.parametriOpzionali2);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
		i++;
		if(this.operatoreUltimoAggiornamento != null) {
	    	arg.setString(i, this.operatoreUltimoAggiornamento);
		} else {
			arg.setNull(i, java.sql.Types.VARCHAR);
		}
    	//fine LP PG190220
