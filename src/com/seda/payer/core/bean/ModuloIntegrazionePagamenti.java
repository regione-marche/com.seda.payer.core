...
public class ModuloIntegrazionePagamenti extends Lifecycle implements Serializable {
    ...
    public ModuloIntegrazionePagamenti() { 
    }

    public ModuloIntegrazionePagamenti(ResultSet data) throws SQLException {
    	if (data == null)
    		return;

    	Calendar cal = Calendar.getInstance();
    	chiaveTransazione = data.getString("MIP_KTRAKTRA");
    	idPortale = data.getString("MIP_KMIPPORT");
    	numeroOperazione = data.getString("MIP_KMIPOPER");
    	xmlPaymentRequest = data.getString("MIP_CMIPPREQ");
    	xmlPaymentData = data.getString("MIP_CMIPPDAT");
    	statoPagamentoCUP = data.getString("MIP_CMIPSTAT");
    	ultimoEsitoNotifica = data.getString("MIP_CMIPUESI");
    	codicePortaleEsterno = data.getString("MIP_CMIPPORT");
    	parametriOpzionali1 = data.getString("MIP_DMIPOPT1");
    	parametriOpzionali2 = data.getString("MIP_DMIPOPT2");
    	cal.setTimeInMillis(data.getTimestamp("MIP_GMIPGAGG").getTime());
    	dataUltimoAggiornamento = cal;
    	codiceSocieta = data.getString("MIP_CSOCCSOC");
    	codiceUtente = data.getString("MIP_CUTECUTE");
    	chiaveEnte = data.getString("MIP_KANEKENT");
    	commitNotifica = data.getString("MIP_FMIPNOTI");
    }
