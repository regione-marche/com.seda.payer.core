...
public class LogWinDao extends BaseDaoHandler {

	//LP PG22XX06 - 20220607 private Logger log = Logger.getLogger(LogWinDao.class);

	public LogWinDao(Connection connection, String schema) {
		super(connection, schema);
		//LP PG22XX06 - 20220607 log.debug("LogWinDao: Dao avviato");
	}
	

	public LogWin doDetail(BigInteger idLog, String tagSuffissoTabella) throws DaoException {
		CallableStatement callableStatement = null;
		ResultSet data = null;
		
		//LP PG22XX06 - 20220607 log.debug("LogWinDao: \n\tOP: DETAIL \n\t ROUTINE: " + 
		//LP PG22XX06 - 20220607 		Routines.PGWINSP_SEL + "\n\tPARAM: idLog:" + idLog);
		try {
			callableStatement = prepareCall(Routines.PGWINSP_SEL.routine());
			LogWin bean = new LogWin();
			bean.setIdLog(idLog);
			bean.setTagSuffissoTabella(tagSuffissoTabella);
			bean.load(callableStatement);
			if (callableStatement.execute()) {
				//LP PG22XX06 - 20220607 System.out.println("LogWinDao: esecuzione chiamata db");
				data = callableStatement.getResultSet();
				if (data.next()){
					//LP PG22XX06 - 20220607 System.out.println("LogWinDao: Stai ritornando un valore");
					return new LogWin(data);
				}
			}
			return null;
	...
	public void doWebRowSets(LogWin bean, int rowsPerPage, int pageNumber, String order) throws DaoException {
		CallableStatement callableStatement = null;
		try	{
			//LP PG22XX06 - 20220607 log.debug("LogWinDao: OP: doWebRowSets\n\tOBJ: " + bean.toString());
			callableStatement = prepareCall(Routines.PGWINSP_LST.routine());
	...
	public void doInsert(LogWin bean) throws DaoException {
		CallableStatement callableStatement = null;
		try {
			makeTabBackup(bean.getOperatoreInserimento());
			//LP PG22XX06 - 20220607 log.debug("LogWinDao: \n\tOP: INSERT \n\t ROUTINE: " + Routines.PGWINSP_INS + "\n\tOBJ: " + bean.toString());
			callableStatement = prepareCall(Routines.PGWINSP_INS.routine());
...
//		log.debug("\nLogWinDao: \n\tSchema: " + nameSchema);
		try {
			String tabBackup = "PGWINTB_" +  tagBackup;
			...
			if(callableStatement.getInt(3) == 0) {
				...
				callableStatement.setString(1, "PGWINTB");
...
//			log.debug("LogWinDao settimana corrente dataInizio: " + dataInizioCorrente + " dataFine: " + dataFineCorrente);
